...
def is_real_integration(name: str) -> bool:
    return os.path.isfile(f"core/homeassistant/components/{name}/__init__.py")
integrations = [x for x in os.listdir("core/homeassistant/components") if is_real_integration(x)]
...
def has_feature(name: str, featur: str) -> bool:
    return os.path.isfile(f"core/homeassistant/components/{name}/{featur}.py")
df = df.sort_index()
for integration_name, integration in df.iterrows():
    ...
    with open(f"core/homeassistant/components/{integration_name}/manifest.json") as manifest_file:
    ...
    has_tests = os.path.exists(f"core/tests/components/{integration_name}")
    df.at[integration_name, "has_tests"] = has_tests
    # if has_tests:
    #     try:
    #         p = subprocess.Popen(["pytest", f"core/tests/components/{integration_name}", f"--cov=homeassistant.components.{integration_name}"], stdout=subprocess.PIPE)
